...
- Execute `make deploy` to deploy the chaincode to the `fabric-samples` chaincode folder.  Note: you may need edit the [`Makefile`](https://github.com/yxuco/flogo-enterprise-app/blob/master/marble-app/Makefile) to set `CC_DEPLOY` if `fabric-samples` are not downloaded to the default location under `$GOPATH`.
